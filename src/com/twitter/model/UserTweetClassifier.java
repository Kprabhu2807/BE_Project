...
public abstract class UserTweetClassifier<T, K> implements Probability<T, K> {
    ...
    private Queue<ClassificationData<T, K>> memoryQueue;

    public UserTweetClassifier() {
        this.reset();
    }
    ...
    public void reset() {
        this.featureCountPerCategory =
                new Hashtable<K, Dictionary<T,Integer>>(
                        UserTweetClassifier.INITIAL_CATEGORY_DICTIONARY_CAPACITY);
        this.totalFeatureCount =
                new Hashtable<T, Integer>(
                        UserTweetClassifier.INITIAL_FEATURE_DICTIONARY_CAPACITY);
        this.totalCategoryCount =
                new Hashtable<K, Integer>(
                        UserTweetClassifier.INITIAL_CATEGORY_DICTIONARY_CAPACITY);
        this.memoryQueue = new LinkedList<ClassificationData<T, K>>();
    }
    ...
    public float featureWeighedAverage(T feature, K category,
            Probability<T, K> calculator) {
        return this.featureWeighedAverage(feature, category,
                calculator, 1.0f, 0.5f);
    }

    public float featureWeighedAverage(T feature, K category,
            Probability<T, K> calculator, float weight) {
        return this.featureWeighedAverage(feature, category,
    ...
    public void learn(K category, Collection<T> features) {
        this.learn(new ClassificationData<T, K>(features, category));
    }

    
    public void learn(ClassificationData<T, K> classification) {
        ...
        if (this.memoryQueue.size() > this.memoryCapacity) {
            ClassificationData<T, K> toForget = this.memoryQueue.remove();
    ...
    public abstract ClassificationData<T, K> classify(Collection<T> features);
